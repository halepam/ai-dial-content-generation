...
def start() -> None:
    ...
    endpoint = DIAL_ENDPOINT + "/openai/deployments/{model}/chat/completions"
    client = DialModelClient(endpoint, "gpt-4o", API_KEY)
    ...
    messages = [
        ContentedMessage(
            ...
            content=[
                ...
                ImgContent(
                    image_url=ImgUrl(url="data:image/png;base64," + base64_image)
                ),
            ],
        ),
    ]
    message = client.get_completion(messages)
